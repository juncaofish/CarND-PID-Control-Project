      ...
      if (length && length > 2 && data[0] == '4' && data[1] == '2') {
        ...
        if (s != "") {
          ...
          if (event == "telemetry") {
            ...
            steer_value = max(min(pid_steer.TotalError(), 1.0), -1.0);

            pid_speed.UpdateError(cte);
            speed_value = max(min(pid_speed.TotalError(), 0.6), -0.6);
            // DEBUG
            std::cout << "CTE: " << cte << " Steering Value: " << steer_value << std::endl;
            std::cout << "CTE: " << cte << " Speed Value: " << speed_value << std::endl;

            json msgJson;
            msgJson["steering_angle"] = steer_value;

            // speed value should adjust according to cte
            msgJson["throttle"] = 1 - abs(speed_value);
